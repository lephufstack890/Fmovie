...
const MovieEditPage = () => {
  ...
  const [editMovieMutation, ] = useEditMoviesMutation()
  ...
  const categoryIds: number[] = movie?.data?.id_category.map((category: { id: number }) => category.id) || [];
  const timeShowIds: number[] = movie?.data?.time_shows.map((timeshow: { id: number }) => timeshow.id) || []
  const dayMovieIds: number[] = movie?.data?.day_movies.map((daymovie: { id: number }) => daymovie.id) || []

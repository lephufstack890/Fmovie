...
import { useAddPaymentMutation } from "@/services/payment/payments.services"
...
const Payment = ({ quantity, totalPriceProps, selectedSeatsId, selectedSeats, infoShowtime, handlerPrevious }) => {
    const {id} = useParams()
    ...
    const [chooseSeat, { isLoadingChooseSeat }] = useChooseSeatMutation();
    const [addPaymentMutation, {isLoadingAddPayment}] = useAddPaymentMutation()
    ...
    const handleSubmit = async () => {
        if(selectedBank === null){
            ...
            const formData = {
                ...
                paymentMethod: selectedInput,
                time: timeCurrent,
                seats: selectedSeats,
                order_code: orderNumber,
                name_cinemas: infoShowtime?.cinema.name,
                name_movie: infoShowtime?.movies.name,
                name_room: infoShowtime?.rooms.name,
            ...
            const selectSeat = {
                id_user: user?.id,
                id_seat: selectedSeatsId,
                id_showtime: parseInt(id)
            }
            setCancelSeat(selectSeat);
            try { 
                await chooseSeat(selectSeat);
                await addPaymentMutation(formData).unwrap()
                .then(response => {
                    dispatch(addNewPayment(formData));
        ...
        <div className="row">
            <div className="col-8">
                ...
                {/* <div className="mt-3 d-flex justify-content-end align-items-center">
                    ...
                    <button className='bth bth-primary'>Thanh toán</button>
                </div> */}
            </div>
            <div className="col-4">

...
interface SelectPositionProps {
  handleSeatClick: (seatId: string, seatRow: string) => void;
  setQuantity: (quantity: number) => void;
  setTotalPriceProps: (totalPrice: number) => void;
...
const SelectPosition: React.FC<SelectPositionProps> = ({ handleSeatClick, setQuantity, setTotalPriceProps, setSelectedSeatsId }) => {
  ...
  const [seatNormal, setSeatNormal] = useState<number>(0);
  const [seatVip, setSeatVip] = useState<number>(0);
  const [seatDouble, setSeatDouble] = useState<number>(0);
  ...
  const renderSeats = () => {
    if (!timeshow) return null;
    return timeshow?.data[0].seats.map((seatInfo: TimeShow) => {
      const isSelected = selectedSeats.includes(seatInfo?.id);
      let seatClass = "seat-cell seat-empty  seat-used";
      switch (seatInfo?.nameRow) {
        case "T":
